...
store_affiliations(Name, Host, Affiliations) ->
  ...
  lists:foreach(fun({{U, S, _R}, Affiliation}) ->
			  mnesia:dirty_write(#user_room{us = {U, S}, room = {Name, Host}, affiliation = Affiliation})
					 end, Affiliations).
...
room_jid_to_pid(#jid{luser=RoomName, lserver=MucService}) ->
    room_to_pid({RoomName, MucService}).

-spec room_to_pid(Room :: tuple()) -> {ok, pid()} | {error, not_found}.
room_to_pid(Room) ->
    case mnesia:dirty_read(muc_online_room, Room) of
        [R] ->
