...
init([Host, ServerHost, Access, Room, HistorySize, RoomShaper, Creator, _Nick,
	...
	%% Boris Okner: add server host jid as a dummy owner
	State0 = #state{host = Host,
	...
	State = 
	  	set_affiliation(Creator, owner,
			    set_opts(DefRoomOpts, State0)),

    ?INFO_MSG("Created MUC room ~s@~s by ~s",
              [Room, Host, jlib:jid_to_binary(Creator)]),
    add_to_log(room_existence, created, State),
...
normal_state({route, From, <<>>,
    ...
    NewStateData = route_message(#routed_message{
        allowed = is_user_online(From, StateData) orelse
			%% Boris Okner : allow any affiliated JID to send message to the room
			(get_affiliation(From, StateData) /= none) orelse
    ...
    true ->
        ...
        or ((StateData#state.config)#config.moderated == false) ->
            ...
            true ->
                ...
                Packet1 ->
                lists:foreach(
                  fun({_LJID, #user{jid = FullJID}}) ->
					  case is_subscribed(FullJID, StateData#state.subscriptions, Packet1) of
						true->
                      		ejabberd_router:route(
...
set_affiliation(JID, Affiliation, StateData) ->
    ...
    {Affiliations, Op} = case Affiliation of
		       ...
		       _ ->
                 {?DICT:store(LJID,
                      Affiliation, StateData#state.affiliations), <<"both">>}
		   end,
    room_roster_push(JID, StateData#state.host, StateData#state.room, 
								 Affiliations, Op),
	
    NSD = StateData#state{affiliations = Affiliations},
...
add_online_user(JID, Nick, Role, StateData) ->
	...
	auto_affiliate(JID, StateData#state{users = Users}, member).
...
disco_item(User=#user{jid = Jid, nick=Nick}, RoomJID) ->
    #xmlel{
        name = <<"item">>,
        attrs = [{<<"jid">>, jlib:jid_to_binary(Jid
					%%occupant_jid(User, RoomJID)
						)},
                 {<<"name">>, Nick}]}.
...
room_roster_push(FromJID, _RoomHost, RoomId, Affiliations, Subscription) ->
  lists:foreach(fun({User, _Affiliation}) ->
				roster_push(FromJID, to_jid(User), RoomId, Subscription)		 
