...
start(normal, _Args) ->
  %% Temporarily disable lager log watermark, so any errors on startup are being registered
    error_logger_lager_h:set_high_water(undefined),
	...
	%% Reapply the lager watermark
    {ok, LagerWaterMark} = application:get_env(lager, error_logger_hwm),
    error_logger_lager_h:set_high_water(LagerWaterMark),
    Sup;
start(_, _) ->
    {error, badarg}.
