...
normal_state({route, From, <<>>,
    ...
    {U, _, _} = jlib:jid_tolower(From),
    NewStateData = route_message(#routed_message{
        allowed = (U == <<"bot">>) or (is_user_online(From, StateData) orelse
...
process_groupchat_message(From, #xmlel{name = <<"message">>,
    ...
    Lang = xml:get_attr_s(<<"xml:lang">>, Attrs),
    {U, _, _} = jlib:jid_tolower(From),
    case (U == <<"bot">>) or (is_user_online(From, StateData) orelse
    is_user_allowed_message_nonparticipant(From, StateData)) of
    true ->
        if  U == <<"bot">> -> {FromNick, Role} = {<<"bot">>, moderator};
            true -> {FromNick, Role} = get_participant_data(From, StateData)
        end,
        if
        % Update: Alway allow "bot" user send message in room (no need membership)
        (U == <<"bot">>) or (Role == moderator) or (Role == participant)
...
choose_new_user_strategy(From, Nick, Affiliation, Role, Els, StateData) ->
    {U, _, _} = jlib:jid_tolower(From),
    % Auto allow user with jid "bot" join room
    if  U == <<"bot">> -> allowed;
        true ->
