...
timestamp_to_iso(milisecs, {{Year, Month, Day}, {Hour, Minute, Second, Milisecs}}, Timezone) ->
    Timestamp_string =
        lists:flatten(
          io_lib:format("~4..0w-~2..0w-~2..0wT~2..0w:~2..0w:~2..0w.~3..0w",
                        [Year, Month, Day, Hour, Minute, Second, Milisecs])),
    Timezone_string =
        case Timezone of
            utc -> "Z";
            {Sign, {TZh, TZm}} ->
...
now_to_utc_string_milisecs({MegaSecs, Secs, MicroSecs}) ->
	Milisecond = trunc(MicroSecs/1000),
    {{Year, Month, Day}, {Hour, Minute, Second}} =
        calendar:now_to_universal_time({MegaSecs, Secs, MicroSecs}),
    lists:flatten(
      io_lib:format("~4..0w-~2..0w-~2..0wT~2..0w:~2..0w:~2..0w.~3..0wZ",
                    [Year, Month, Day, Hour, Minute, Second, Milisecond])).
now_to_utc_binary_milisecs(Timestamp) ->
    list_to_binary(now_to_utc_string_milisecs(Timestamp)).
...
datetime_string_to_timestamp(TimeStr) when is_binary(TimeStr) ->
    Time = binary_to_list(TimeStr),
    datetime_string_to_timestamp(Time);
...
get_timestamp() ->
	...
	Milisecs = trunc(MicroSecs/1000),
	{{Year, Month, Day}, {Hour, Minute, Second}} =
        calendar:now_to_universal_time({MegaSecs, Secs, MicroSecs}),
    timestamp_to_iso(milisecs, {{Year, Month, Day}, {Hour, Minute, Second, Milisecs}}, utc).
