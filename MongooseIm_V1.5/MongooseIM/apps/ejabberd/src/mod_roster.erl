            ...
            {ItemsToSend, VersionToSend} =
                ...
                {{value, RequestedVersion}, true, true} ->
                        ...
                        case mnesia:dirty_read(roster_version, US) of
                                ...
                                [] ->
                                        RosterVersion = sha:sha(term_to_binary(os:timestamp())),
...
process_item_set(From, To, #xmlel{attrs = Attrs, children = Els}) ->
    ...
    case JID1 of
        ...
        _ ->
            ...
            F = fun() ->
	Res =  case gateway_jid(JID1, LServer) of
					true ->
						[];
					false ->
						mnesia:read({roster, {LUser, LServer, LJID}})
				   end,
                        Item = case Res of
                                   [] ->
                        ...
                        case Item2#roster.subscription of
                            ...
                            _ ->
			%% Gateway fork	
				case gateway_jid(JID1, LServer) of
					true ->
						void;
					false ->
				mnesia:write(Item2)
                        ...
                        case roster_version_on_db(LServer) of
                                true -> mnesia:write(#roster_version{us = {LUser, LServer}, version = sha:sha(term_to_binary(os:timestamp()))});
                                false -> ok
                        end,
			%% Gateway fork	
			case gateway_jid(JID1, LServer) of
					true when Item3#roster.subscription == none ->
					{Item, Item3#roster{subscription = both}};
					_ ->
					{Item, Item3}
			end
                end,
...
get_subscription_lists(_, User, Server) ->
	...
	%% Quick hack, should be going through subscription hook in mod_muc
		mod_muc:get_room_roster([], US) of
        Items when is_list(Items) ->
            Res = fill_subscription_lists(JID, Items, [], [], []),
			Res;
        _ ->
            {[], [], []}
    end.
...
process_subscription(Direction, User, Server, JID1, Type, Reason) ->
    ...
    LJID = jlib:jid_tolower(JID1),
    F = fun() ->
		Res =  case gateway_jid(JID1, Server) of
					true ->
						[];
					false ->
						mnesia:read({roster, {LUser, LServer, LJID}})
...
process_item_set_t(LUser, LServer, #xmlel{attrs = Attrs,
    ...
    case JID1 of
        ...
        _ ->
			...
			case lists:member(JID1#jid.server, gateways(LServer)) of
					true ->
						void;
					false ->
	                    mnesia:write(Item2)
...
gateways(Host) ->
  [iolist_to_binary(G) || G <- gen_mod:get_module_opt(Host, ?MODULE, gateways, [])].

gateway_jid(Jid, Host) ->
  %%lists:member(Jid#jid.server, gateways(Host)) andalso 
	%%Jid#jid.luser /= [].
	true.
