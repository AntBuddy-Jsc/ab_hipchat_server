...
-export([runLoadTest/1]).
-export([check_acl_rule/4,
		check_bot_company/2]).
...
-ifdef(NO_TRANSIENT_SUPERVISORS).
-define(SUPERVISOR_START_2,
        gen_fsm:start(?MODULE,
                      [Host, ServerHost, Access, Room, HistorySize,
                       RoomShaper, Creator, Nick, DefRoomOpts, Default],
                      ?FSMOPTS)).
-else.
-define(SUPERVISOR_START_2,
        Supervisor = gen_mod:get_module_proc(ServerHost, ejabberd_mod_muc_sup),
...
start(Host, ServerHost, Access, Room, HistorySize, RoomShaper,
      Creator, Nick, DefRoomOpts, Default) ->
    ?SUPERVISOR_START_2.
...
init([Host, ServerHost, Access, Room, HistorySize, RoomShaper, Creator, _Nick,
	...
	State = set_opts(DefRoomOpts, #state{host = Host,
	...
	%% AB#25
	%mod_muc:store_room(Host, Room, make_opts(State1)),
	...
	%% AB#27
	self() ! create_rabbitmq_channel,
	%% AB#43
	add_inactive_member(Creator, State#state.jid),
    {ok, NextState, State1};
...
init([Host, ServerHost, Access, Room, HistorySize, RoomShaper, Creator, _Nick,
	...
	%% AB%25
	%mod_muc:store_room(Host, Room, make_opts(State1)),
	?DEBUG("AB#33: State1 ~p~n", [State1#state.anonymous_domain]),
	save_persistent_room_state(State1),
    ?INFO_MSG("Created MUC room ~s@~s by ~s",
              [Room, Host, jlib:jid_to_binary(Creator)]),
    add_to_log(room_existence, created, State1),
	...
	%% AB#27
	case Default of
        true -> self() ! create_rabbitmq_channel;
        _ -> ok
    end,
    %% AB#43
	add_inactive_member(Creator, State#state.jid),
...
init([Host, ServerHost, Access, Room, HistorySize, RoomShaper, Opts]) ->
    process_flag(trap_exit, true),
    Shaper = shaper:new(RoomShaper),
	Default = mod_muc:check_room_default(ServerHost, Room),
    State = set_opts(Opts, #state{host = Host,
                                  ...
                                  room_shaper = Shaper,
								  default_room = Default}),
	?DEBUG("AB#33: State1 ~p~n", [State#state.anonymous_domain]),
...
initial_state({route, From, ToNick,
    ...
    case  xml:get_path_s(Presence,[{elem, <<"x">>}, {attr, <<"xmlns">>}]) of
        ?NS_MUC ->
            ...
            process_presence(From, ToNick, Presence, StateData, normal_state);
...
%% Boris Okner: subscribe to message types
handle_sync_event({subscribe, JID, ContentTypes, EventTypes}, _From, StateName, #state{subscriptions = Subscriptions} = StateData) ->
  %% TODO: store subscription in state
    {reply, {ok, subscribed}, StateName, 
	 	StateData#state{
						subscriptions = handle_new_subscription(JID, ContentTypes, EventTypes, Subscriptions)}};

handle_sync_event({change_state, NewStateData}, _From, StateName, _StateData) ->
    {reply, {ok, NewStateData}, StateName, NewStateData};

handle_sync_event(get_affiliations, _From, StateName, StateData) ->
	{reply, ?DICT:to_list(StateData#state.affiliations), StateName, StateData};
handle_sync_event(get_member_online, _From, StateName, StateData) ->
    {reply, ?DICT:to_list(StateData#state.users), StateName, StateData};
...
%% AB#22
handle_info({save_company_room, ServerHost, Room, Default}, StateName, StateData) ->
	case (StateData#state.config)#config.persistent of
        true ->	
			mod_muc:save_company_room(ServerHost, Room, Default);
		false ->
			nothing_to_do
	end,
    {next_state, StateName, StateData};
handle_info({destroyed_default_room, SubEle}, _StateName, StateData) ->
	destroy_room(SubEle, StateData),
    {stop, shutdown, StateData};
%% AB#27
handle_info(create_rabbitmq_channel, StateName, StateData) ->
	ServerHost = StateData#state.server_host,
	RoomJID = get_queue_name_for_broker(StateData),
	ejabberd_hooks:run(init_message_broker_hook, StateData#state.host, [RoomJID, ServerHost]),
...
destroy_temporary_room_if_empty(StateData=#state{config=C=#config{}}) ->
    case (not C#config.persistent) andalso is_empty_room(StateData) of
        true ->
			%% AB#33
			spawn(fun() ->
				HookUrls = ejabberd_config:get_local_option({webhook_url, StateData#state.server_host}),
				lists:foreach(fun(HookUrl) ->
					Url = HookUrl ++ "destroy",
					Body = "uuid=" ++ binary_to_list(StateData#state.room),
...
process_presence_unavailable(From, Packet, StateData) ->
    case is_user_online(From, StateData) of
        true ->
			...
			case ejabberd_config:get_local_option({host_type,
										StateData#state.server_host}) of
                support ->
						{Ts, _} = jlib:get_timestamp(),
			            ejabberd_hooks:run(update_room_status, StateData#state.server_host,
                                    [update_inactive, From,
                                    StateData#state.server_host,
                                    StateData#state.jid, <<"0">>, Ts]);
				_ ->
					no_update_inactive
			end,
...
set_affiliation(JID, Affiliation, StateData)
	...
	{Affiliations, Op} = case Affiliation of
				none ->
					{?DICT:erase(LJID,
						StateData#state.affiliations), <<"remove">>};
				_ ->
					case JID#jid.luser of
						<<"bot">> ->
							ejabberd_hooks:run(bot_add_new_user, JID#jid.lserver,
								[JID, StateData#state.room, calendar:now_to_universal_time(os:timestamp())]);
						_ ->
							ok
					end,
					{?DICT:store(LJID,
						Affiliation,
						StateData#state.affiliations), <<"both">>}
			end,
	set_affiliation_by_roster_push(JID, Affiliations, StateData, Op, Affiliation),
	StateData#state{affiliations = Affiliations}.
		
get_room_by_company(Host, CompanyId) ->
    mod_company_odbc:search_rooms_in_company(Host, CompanyId).
...
set_affiliation_by_roster_push(JID, Affiliations, StateData, Op, Affiliation) ->
	?INFO_MSG("AB#22 Default: ~p~n", [StateData#state.default_room]),
	LJID = jlib:jid_remove_resource(jlib:jid_tolower(JID)),
    case StateData#state.default_room of
        true ->
            room_roster_push(JID, StateData#state.host, StateData#state.room,
                                        Affiliations, Op),
            case Op of
                <<"remove">> ->
                    %% If remove a user from a default room (org)
                    %% user will be removed from all rooms that belongs to org
                    %% update information of user to user_room(send roster push)
                    %% all rooms belongs to company
                    RoomsOfCompany = get_room_by_company(StateData#state.server_host, StateData#state.room),
                    RoomNameOfCompany = lists:map(fun({Room}) ->
                                        {list_to_binary([Room, <<"_">>, StateData#state.room])}
                                end, RoomsOfCompany),
                    RoomsOfUser = mod_muc_company:get_rooms_of_user({JID#jid.luser, JID#jid.lserver}),
                    % all rooms which user joined
                    RoomNameOfUser = lists:map(fun({RoomName, _}) ->
                                                            {RoomName}
                                                end, RoomsOfUser),
                    ?DEBUG("AB#22 RoomNameOfCompany ~p~n RoomNameOfUser ~p~n", [RoomNameOfCompany, RoomNameOfUser]),
                    RoomSets = sets:intersection(sets:from_list(RoomNameOfUser),
                                                sets:from_list(RoomNameOfCompany)),
                    RoomList = sets:to_list(RoomSets),
                    ?INFO_MSG("AB#22 RoomList ~p~n", [RoomList]),
					mod_muc:store_muc_room(JID, StateData#state.host, StateData#state.room, RoomList),
					mod_muc:store_affiliations_to_default_room(StateData#state.host,
													StateData#state.affiliations, LJID, Affiliation, RoomList);
                _ -> ok
            end;
        _ -> ok
    end,
    ?DEBUG("AB#18 Affiliations ~p~n ", [Affiliations]),
	%% AB#30
	case (StateData#state.config)#config.persistent of
		true ->
			mod_muc:store_affiliations(StateData#state.room, StateData#state.host,
                        StateData#state.affiliations, LJID, Affiliation);
		_ ->
			nothing_to_do
	end.
...
get_affiliation(JID, StateData) ->
    ...
    Res =
	case check_acl_rule(StateData#state.server_host, AccessAdmin, 
									JID, StateData#state.room) of
        allow ->
        owner;
        _ ->
        LJID = jlib:jid_tolower(JID),
...
%%AB#40
check_acl_rule(ServerHost, AccessAdmin, JID = #jid{}, RoomID) ->
	case (check_bot_company(JID#jid.luser, RoomID) == allow) or
		 (acl:match_rule(ServerHost, AccessAdmin, JID) == allow) of
		true -> 
			allow;
		_Err ->
			false
...
check_acl_rule(ServerHost, AccessAdmin, JID, _RoomID) ->
  case acl:match_rule(ServerHost, AccessAdmin, JID) of
    allow -> 
      allow;
    _Err ->
      false
...
%% check jid is a bot of company
check_bot_company(JID, RoomJID) ->
	case binary:split(JID, [<<"_">>]) of 
		[<<"bot">>, CompanyJID] ->
				case binary:split(RoomJID, [<<"_">>]) of
					[_ , CompanyId] ->
						if (CompanyId == CompanyJID) -> 
							allow;
							true -> not_bot_company
						end;
					[CompanyJID] ->
						allow;
					[_Comp] -> not_bot_company;
					_ -> none
				end;
		_ -> none
	end.
...
dict_foreach_value(F, Users) ->
    ?DICT:fold(fun(_LJID, User, _) -> 
		F(User) end, undefined, Users).
...
add_online_user(JID, Nick, Role, StateData) ->
	...
	%%AB#27: Send list user offline in room
    send_online_user(Users, StateData),
    auto_affiliate(JID, StateData#state{users = Users, sessions = Sessions}, member).
    %StateData1#state{users = Users, sessions = Sessions}.
...
%% HUE_TRAN
is_nick_exists(_Nick, _StateData)->
   false.
...
add_new_user(From, Nick,
    ...
    Host_Type = ejabberd_config:get_local_option({host_type, StateData#state.server_host}),
    if (From#jid.lserver /= StateData#state.server_host) andalso (Host_Type == support) ->
      %% Set member for supporter
      ejabberd_hooks:run(count_unread_message_kite_room, 
                                                StateData#state.server_host,
                                                [From, StateData#state.jid]),
      NewState = set_affiliation(From, member, StateData),
      save_persistent_room_state(NewState),
      NewState2 = add_user_presence(
                  From, Packet,
                  add_online_user(From, Nick, Role, NewState)),
      send_new_presence(From, NewState2),
      send_existing_presences(From, NewState2),
      NewState2;
      true ->
        case choose_new_user_strategy(From, Nick, Affiliation, Role, Els, StateData) of
            ...
            allowed ->
    			...
    			%% AB#14 <<
    			case NewState#state.anonymous_domain of
    				true ->
    					send_existing_presences(From, NewState);
    				_ -> 
    					nothing_to_do
...
send_new_presence(NJID, Reason, StateData) ->
	%% AB#33 >>
	%% AB#14: remove presence when join room >>
	case StateData#state.anonymous_domain of
		true ->
			send_new_presence_to(NJID, Reason, ?DICT:to_list(StateData#state.users), StateData);
		_ ->
			InfoJID = lists:filter(fun({_, Info}) -> NJID == Info#user.jid end, ?DICT:to_list(StateData#state.users)),
			send_new_presence_to(NJID, Reason, InfoJID, StateData)
	end.
	%% AB#14 <<
...
send_new_presence_to(NJID, Reason, Receivers, StateData) ->
    ...
    lists:foreach(
      fun({_LJID, Info}) ->
		  ...
		  Timestamp = case ejabberd_config:get_local_option({host_type, 
								StateData#state.server_host}) of
				support ->
					{Ts, Utc} = jlib:get_timestamp(),
					[#xmlel{name = <<"timestamp">>, children = [#xmlcdata{content = Ts ++ Utc}]}];
				_ ->
					[]
		  end,
          Packet = xml:append_subtags(
             ...
             [#xmlel{name = <<"x">>, attrs = [{<<"xmlns">>, ?NS_MUC_USER}],
                     children = [#xmlel{name = <<"item">>, attrs = ItemAttrs,
                                        children = ItemEls} | Status2] ++ Timestamp }]),
			  ejabberd_router:route(
				jlib:jid_replace_resource(StateData#state.jid, Nick),
				Info#user.jid,
				Packet)
...
send_invitation(From, To, Reason, StateData=#state{host=Host, server_host=ServerHost, jid=RoomJID}) ->
	?INFO_MSG("AB#19: send_invitation From ~p~n To ~p~n RoomJID ~p~n ", [From, To, RoomJID]), 
    ejabberd_hooks:run(invitation_sent, Host, [Host, ServerHost, RoomJID, From, To, Reason]),
    ejabberd_hooks:run(push_notification_invite_room, Host, 
                                            [From, To, RoomJID]),
	...
	%% AB#43
	  %add_inactive_member(To, RoomJID),
    ejabberd_router:route(
        RoomJID,
        To,
...
add_message_to_history(FromNick, FromJID, Packet, StateData) ->
    ...
    case FromJID#jid.luser of
        <<"bot">> ->  
				ejabberd_hooks:run(bot_send_message, FromJID#jid.lserver,
                        [FromJID, StateData#state.room, Packet, TimeStamp]);
        _ -> ok
    end,	
	...
	case (xml:get_subtag(Packet, <<"paused">>) == false)
        andalso (xml:get_subtag(Packet, <<"composing">>) == false)
        andalso (xml:get_subtag(Packet, <<"delay">>) == false)
		andalso (xml:get_subtag(Packet, <<"pause">>) == false) of
			true ->
				ejabberd_hooks:run(room_packet, StateData#state.host,
                       [FromNick, FromJID, StateData#state.jid, TSPacket]);
			false ->
				false
	end,
				  ...
				  fun(E, SD) ->
					  case catch (
						 case E of
							 ...
							 ejabberd_hooks:run(delete_member_room, StateData#state.server_host,
...
send_kickban_presence(JID, Reason, Code, StateData) ->
	...
	case JID#jid.luser of
        <<"bot">> ->
            ejabberd_hooks:run(bot_remove_user, JID#jid.lserver, 
                                [JID, StateData#state.room, calendar:now_to_universal_time(os:timestamp())]);
        _ ->
			ok
    end,
...
send_kickban_presence(JID, Reason, Code, NewAffiliation, StateData) ->
	?INFO_MSG("AB#19: send_kickban_presence JID ~p~n NewAffiliation ~p~n", [JID, NewAffiliation]),
    ...
    owner ->
        ...
        [#xmlel{name = <<"destroy">>} = SubEl1] ->
            ?INFO_MSG("Destroyed MUC room ~s by the owner ~s",
                  [jlib:jid_to_binary(StateData#state.jid), jlib:jid_to_binary(From)]),
            add_to_log(room_existence, destroyed, StateData),
		    case From#jid.luser of
				<<"bot">> ->
		            ejabberd_hooks:run(bot_destroy_room, From#jid.lserver,
                            [From, StateData#state.room, calendar:now_to_universal_time(os:timestamp())]);
				_ -> ok
		    end,
...
make_opts(StateData) ->
    ...
    [
	 ...
	 %%AB#33
	 {anonymous_domain, StateData#state.anonymous_domain}
    ].
...
remove_each_occupant_from_room(DestroyEl, StateData) ->
	?INFO_MSG("AB#40: DestroyEl: ~p~n", [DestroyEl]),
    Packet = presence_stanza_of_type_unavailable(DestroyEl),
	F = fun(_User=#user{jid=UserJID}) ->
			LJID = jlib:jid_remove_resource(jlib:jid_tolower(UserJID)),
			%% AB#22
			Host = StateData#state.host,
			case StateData#state.default_room of
				true ->
					%% delete all rooms that belongs to default toom
					RoomList = get_room_by_company(StateData#state.server_host, 
													StateData#state.room),
					?INFO_MSG("AB#40: RoomList: ~p~n", [RoomList]),
					lists:foreach(fun({Room}) ->
						RoomId = list_to_binary([Room, <<"_">>, StateData#state.room]),
						%% send terminate message to delete process of room
						{ok, Pid} = mod_muc:room_jid_to_pid(RoomId, Host),
						RoomJID = <<RoomId/binary, "@", Host/binary>>,
						SubRoomDestroyEle = {xmlel,<<"destroy">>,
												[{<<"jid">>, RoomJID}],
												[{xmlel,<<"reason">>,[],[]}]},
						Pid ! {destroyed_default_room, SubRoomDestroyEle}
							end, RoomList),
					mod_company_odbc:delete_company_room(StateData#state.server_host, 
														StateData#state.room, StateData#state.room),
					%%AB#27
					case (StateData#state.config)#config.persistent of
						true ->
				            RoomJID = get_queue_name_for_broker(StateData),
							ejabberd_hooks:run(destroy_queue_hook, Host, 
													[RoomJID, StateData#state.server_host]);
						false -> ok
					end,
					room_roster_push(
				              UserJID, StateData#state.host, StateData#state.room,
				              ?DICT:erase(LJID, StateData#state.affiliations), <<"remove">>);
				_ ->
					[RId | CId] = binary:split(StateData#state.room, [<<"_">>]),
					mod_company_odbc:delete_company_room(StateData#state.server_host, CId, RId)
			end
		end,
...
element_size(El) ->
    case catch size(xml:element_to_binary(El)) of
      {'EXIT', Err} -> 
        ?INFO_MSG("Error: ~p; El: ~p~n", [Err, El]), 
        10;
      Size -> Size
    end.
...
route_message(#routed_message{allowed = true, type = Type,
    from = From, packet = Packet, lang = Lang}, StateData) when Type == <<"groupchat">> orelse Type == <<"event">> ->
    Activity = get_user_activity(From, StateData),
    Now = now_to_usec(os:timestamp()),
...
route_invitation({error, Error}, From, Packet, _Lang, StateData) ->
    ...
    ejabberd_router:route(StateData#state.jid, From, Err),
    StateData;
route_invitation({ok, IJIDs}, From, Packet, _Lang, StateData0) ->
                      ...
                      case get_affiliation(IJID, StateData) of
                          none ->
                              NSD = set_affiliation(
                                      ...
                                      member,
                                      StateData),
							  %% AB#43: hook to update inactive status of room
							  add_inactive_member(IJID, StateData0#state.jid),
							  case (NSD#state.config)#config.persistent of
                                  true ->
                                      mod_muc:store_room(
                                        NSD#state.host,
                              ...
                              ejabberd_hooks:run(push_notification_invite_room, NSD#state.host, 
                                                    [From, IJID, StateData0#state.jid]),
                              NSD;
                          _ ->
                              StateData
                      end
...
route_iq(#routed_iq{iq = #iq{type = Type, xmlns = ?NS_DISCO_ITEMS, lang = Lang},
    ...
    do_route_iq(Res, Routed, StateData);
%% AB#43
route_iq(#routed_iq{iq = #iq{type = Type, xmlns = ?NS_MAM_UPDATE_STATUS, sub_el = SubEl},
    from = From} = Routed, StateData) ->
    Res = process_iq_update_status(From, Type, SubEl, StateData),
    do_route_iq(Res, Routed, StateData);
route_iq(#routed_iq{iq = #iq{type = Type, xmlns = ?NS_MAM_CURRENT_TIME, sub_el = SubEl},
    from = From} = Routed, StateData) ->
    Res = process_iq_disco_current_time(From, Type, SubEl, StateData),
    do_route_iq(Res, Routed, StateData);
...
%% Boris Okner
%% "Room roster" roster_push
roster_push(EntryJid, ToJid, Group, Subscription) -> 
  EntryItem = 
	[{xmlel, <<"item">>, 
	  [{<<"jid">>, jlib:jid_to_binary(jlib:jid_remove_resource(EntryJid))}, 
	   {<<"name">>,  EntryJid#jid.luser},
	   {<<"subscription">>, Subscription}
	  ],
	  [
	   #xmlel{name = <<"group">>,
                                       children = [#xmlcdata{content = Group}]}
	   ]}],
  IQ = #iq{type = set, sub_el = [{xmlel, <<"query">>, [{<<"xmlns">>, ?NS_ROSTER}], EntryItem}]},
  ejabberd_router:route(ToJid, jlib:jid_remove_resource(ToJid), jlib:iq_to_xml(IQ)).

room_roster_push(FromJID, RoomHost, RoomId, Affiliations, Subscription) ->
	LJID = jlib:jid_remove_resource(jlib:jid_tolower(FromJID)),
	lists:foreach(fun({User, _Affiliation}) ->
				RosterJID = to_jid(User),
				roster_push(FromJID, RosterJID, RoomId, Subscription),
				if RosterJID /= FromJID -> 
					roster_push(RosterJID, FromJID, RoomId, Subscription);
					true -> ok
				end
			end, dict:to_list(Affiliations)).

to_jid({User, Server, Resource}) ->
	jlib:make_jid(User, Server, Resource). 

%% Boris Okner :  auto-affiliation
auto_affiliate(JID, StateData, Affiliation) ->
  case get_affiliation(JID, StateData) of
	owner -> %% Do not change owner's affiliation
	  StateData;
	Affiliation -> %% requested affiliation is the same as existing, ignore
	  StateData;
	_Aff ->
	  set_affiliation(JID, Affiliation, StateData)
  end.
...
handle_new_subscription(JID, <<>>, <<>>, Subscriptions) ->
  ?DICT:erase(JID, Subscriptions);


handle_new_subscription(JID, ContentTypes, EventTypes, Subscriptions) ->
  ?DICT:store(JID, {comma_delimited(ContentTypes),					 
					comma_delimited(EventTypes)},
  			Subscriptions).

is_subscribed(Jid, Subscriptions, #xmlel{attrs = Attrs} = _Packet) ->
  ContentTypes = xml:get_attr_s(<<"content-types">>, Attrs),
  EventTypes = xml:get_attr_s(<<"event-types">>, Attrs),  
  case ?DICT:find(Jid, Subscriptions) of
	error when ContentTypes == <<>> andalso EventTypes == <<>> ->
	  %% No subscriptions, allow messages with no content types and no event types specified
		true;
	error ->
	  false;
	{ok, {S_ContentTypes, S_EventTypes}} ->
	  not (sets:is_element(<<"none">>, S_ContentTypes) 
		  andalso sets:is_element(<<"none">>, S_EventTypes)) %% No messages for the subscriptions with both "none"
	  andalso
		(
		(ContentTypes == <<>> andalso EventTypes == <<>>) %% Still allow messages with omitted content/event types
  		  orelse
	  	  (not sets:is_disjoint(comma_delimited(ContentTypes), %% No messages with content/event type that were not subscribed to
							S_ContentTypes)) 
		  orelse (not sets:is_disjoint(comma_delimited(EventTypes), 
										S_EventTypes))
		)
  end.
	
%% Parse comma-delimited binary list
comma_delimited(Data) ->
  sets:from_list(binary:split(Data, <<",">>, [global])).
...
runLoadTest(MessageCount) ->
  gen_server:call(mod_count_message, reset),
  gen_server:call(mod_send_message, reset), 
  Rooms = ets:tab2list(muc_online_room),
  lists:foreach( fun(#muc_online_room{pid = Pid}) ->
    %erlang:send_after(?INTERVAL, Pid, {trigger, MessageCount})
    Pid ! {trigger, MessageCount}
  end, Rooms ).

get_queue_name_for_broker(StateData) ->
    Room = StateData#state.room,
    ServerHost = StateData#state.server_host,
    RoomJID = <<Room/binary, ".", ServerHost/binary>>.

send_online_user(Users, StateData) ->
	case StateData#state.default_room of
		false ->
			Room = StateData#state.room,
			Service = StateData#state.server_host,
			Host = StateData#state.host,
			RoomJID = <<Room/binary, "@", Host/binary>>,
			QueueSender = case StateData#state.default_room of
				true ->
					<<Room/binary, ".", Service/binary>>;
				_ ->
					R = lists:last(binary:split(Room, <<"_">>)),
					<<R/binary, ".", Service/binary>>
			end,
			ListMember = lists:map(fun(User) -> {FullJID, _} = User,
								{UserId, Server, _} = FullJID,
								<<UserId/binary, "@", Server/binary>>
						end, ?DICT:to_list(Users)),
			ejabberd_hooks:run(presence_online_hook, Host,
									[Service, ListMember, RoomJID, QueueSender]);
			%sender_to_rabbitmq:send_room_presences_online(Service, ListMember, RoomJID, QueueSender);
		_ ->
			nothing
	end.

%% AB#43
process_iq_update_status(_From, get, _Routed, _StateData) ->
    {error, ?ERR_NOT_ALLOWED};
process_iq_update_status(From, set, Routed, StateData) ->
	Status = xml:get_subtag_cdata(Routed, <<"status">>),	
	{Ts, _} = jlib:get_timestamp(),
	case is_user_online(From, StateData) of
		true ->
			case Status of
				<<"1">> ->
					...
					Stanza = mod_antbuddy_stanza:notify_message(),
					ejabberd_router:route(jlib:jid_replace_resource(
										StateData#state.jid, From#jid.luser), 
										jlib:jid_remove_resource(From), 
										Stanza),
					ejabberd_hooks:run(update_room_status, StateData#state.server_host,
													[update_active, From, 
													StateData#state.server_host,
													StateData#state.jid, Status, Ts]);
				_ ->
					ejabberd_hooks:run(update_room_status, StateData#state.server_host,
											[update_inactive, From, 
											StateData#state.server_host, 
			...
			{result, [], StateData};
		false ->
      ?ERROR_MSG("#43 Not Allow From: ~p, RoomJID: ~p, Routed:~p~n", [From, StateData#state.jid, Routed]),
			{error, ?ERR_NOT_ALLOWED}
	end.

add_inactive_member(JID, RoomJID) ->
		From = jlib:make_jid(JID#jid.luser, JID#jid.lserver, <<>>),
		...
		%% AB#43: hook to update inactive status of room
		{Ts, _} = jlib:get_timestamp(),
    BinTs = muc_util:to_bin(Ts),
		ejabberd_hooks:run(update_room_status, Server,
					[add_status_active, From, Server, RoomJID, <<"0">>, BinTs]).

process_iq_disco_current_time(_From, set, _Routed, _StateData) ->
    {error, ?ERR_NOT_ALLOWED};
process_iq_disco_current_time(From, get, Routed, StateData) ->
	{Ts, Utc} = jlib:get_timestamp(),
	Res = [#xmlel{name = <<"item">>,
				attrs = [{<<"current_time">>, Ts ++ Utc}]}],
