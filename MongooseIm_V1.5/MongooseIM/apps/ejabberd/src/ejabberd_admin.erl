...
%                       args = [], result = {res, rescode}},
     ...
     #ejabberd_commands{name = reload_cluster, tags = [server],
                        ...
                        args = [], result = {res, restuple}},
    #ejabberd_commands{name = get_sessions_num, tags = [sessions],
                      desc = "Get total of sessions", 
                      module = ?MODULE, function = get_total_sessions_number,
                      args = [], result = {res, integer}}
    ].
...
-spec get_total_sessions_number() -> {ok, integer()}.
get_total_sessions_number() ->
	TotalSessions = ejabberd_sm:get_total_sessions_number(),
	TotalSessions.
