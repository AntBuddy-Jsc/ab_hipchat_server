            ...
            case (JID /= error) andalso
                ...
                true ->
                    ...
                    case ejabberd_auth:check_password_with_authmodule(
						...
						%% AB#43
						% ejabberd_hooks:run(c2s_login_success, StateData#state.server, 
						% 								[self(), StateData#state.server, JID, Fs1, Ts1]),
...
handle_info({route, From, To, Packet}, StateName, StateData) ->
	...
	case Name of
		...
		case IQ of
            ...
            %% AB_60
			#iq{xmlns = ?NS_MAM_KITE_ACTIVE_ROOM} ->
				process_kite_room_active(From, To, IQ),
				{false, Attrs, StateData};
		...
		case privacy_check_packet(StateData, From, To, Packet, in) of
		    allow ->
				%% AB#43
                Host_Type = ejabberd_config:get_local_option({host_type, To#jid.lserver}),
				%% check if user is active in room or roster
				%% by using #state.pres_active that saves all object user is active
                AckAttrs = case (xml:get_subtag(Packet, <<"paused">>) == false)
                    andalso (xml:get_subtag(Packet, <<"composing">>) == false)
                    andalso (xml:get_subtag(Packet, <<"delay">>) == false)
                    andalso (xml:get_subtag(Packet, <<"pause">>) == false)
                    andalso (Host_Type /= instant) of
                        true ->
            				case xml:get_attr_s(<<"type">>, Attrs) of
            					<<"chat">> ->
            						?DEBUG("AB#43: Active: ~p~n", [?DICT:to_list(StateData#state.pres_active)]),
            						BFrom = jlib:jid_to_binary(From),
            						if
            							BFrom == From#jid.lserver ->
            									Attrs;
            							true ->
            								Ack = case ?DICT:find(From#jid.luser, 
            													StateData#state.pres_active) of
            									{ok, _} -> <<"1">>;
            									_ -> <<"0">>
            								end,
            								lists:append(Attrs, [{<<"ack">>, Ack}])
            						end;
            					<<"groupchat">> ->
            						Ack = case ?DICT:find(From#jid.luser, StateData#state.pres_active) of
            							{ok, _} -> <<"1">>;
            							_ -> <<"0">>
            						end,
            						lists:append(Attrs, [{<<"ack">>, Ack}]);
...
%% AB#43
%% In data process of JID: StateData.active_member
%% {key = Roster.luser, value = set(JID.lresource)
handle_info({update_active_members, To, JID}, StateName, StateData) ->
	Active = case ?DICT:find(To#jid.luser, 
						StateData#state.pres_active) of
		{ok, Values} ->
			case lists:member(JID#jid.lresource, Values) of
				true ->
					StateData#state.pres_active;
				false ->
					NewValues = [JID#jid.lresource | Values],
					?DICT:store(To#jid.luser, NewValues,
									StateData#state.pres_active)
			end;
		_ ->
			?DICT:store(To#jid.luser, [JID#jid.lresource], 
									StateData#state.pres_active)	
	end,
    ?DEBUG("#86: read-unread: active_members: ~p",[Active]),
    {next_state, StateName, StateData#state{pres_active = Active}};
handle_info({update_inactive_members, To, JID}, StateName, StateData) ->
	Active = case ?DICT:find(To#jid.luser, 
                        StateData#state.pres_active) of
		{ok, Values} ->
			NewValues = Values -- [JID#jid.lresource],
			case length(NewValues) of
				0 ->
					?DICT:erase(To#jid.luser, StateData#state.pres_active);
				_ ->
					?DICT:store(To#jid.luser, NewValues,
                                    StateData#state.pres_active)
			end;
    ...
    ?DEBUG("#86: read-unread:remaining active members after remove: ~p",[Active]),
    {next_state, StateName, StateData#state{pres_active = Active}};
...
handle_info({http, Result}, StateName, StateData) ->
    ?DEBUG("POST respond: ~p", [Result]),
    fsm_next_state(StateName, StateData);
		...
		_ ->
		    ?INFO_MSG("(~w) Close session for ~s",
			      [StateData#state.socket,
			       jlib:jid_to_binary(StateData#state.jid)]),
			%% AB#43
			ejabberd_hooks:run(c2s_logout, StateData#state.server,
									[StateData#state.server, 
									StateData#state.jid]),
		    EmptySet = ?SETS:new(),
		    case StateData of
...
presence_update(From, Packet, StateData) ->
	...
	_ ->
		...
		if
            ...
            ejabberd_hooks:run(c2s_login_success, NewStateData#state.server, 
                [self(), NewStateData#state.server,  NewStateData#state.jid, 
                            ?SETS:to_list(NewStateData#state.pres_f), 
                            ?SETS:to_list(NewStateData#state.pres_t)]),
...
process_kite_room_active(_, _, #iq{type = get}) ->
    not_allowed_method;
process_kite_room_active(From, To, #iq{type = set, sub_el = SubEl} = IQ) ->
    Type = xml:get_subtag_cdata(SubEl, <<"type">>),
    {Timestamp, _} = jlib:get_timestamp(),
    Item = xml:get_subtag_cdata(SubEl, <<"item">>),
    RoomJID = jlib:binary_to_jid(Item),
    ?DEBUG("#86: signal from kite server: JID: ~p; ObjectId: ~p; 
        Status: ~p; Timestamp: ~p; Server: ~p", 
                     [To, RoomJID, Type, Timestamp, To#jid.lserver]),
    case binary_to_atom(Type, utf8) of
        update_active -> 
            ejabberd_hooks:run(update_room_status, To#jid.lserver,
                                [update_active, To, 
                                To#jid.lserver,
                                RoomJID, <<"1">>, Timestamp]);
        update_inactive -> 
        ...
        add_status_active -> 
            ejabberd_hooks:run(update_room_status, To#jid.lserver,
                                [add_status_active, To, To#jid.lserver, 
                                RoomJID, <<"0">>, Timestamp]);
        _ ->
            no_match_type
    end.
